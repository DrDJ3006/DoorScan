    ...
    if len(sys.argv) > 3:
        try:
            if sys.argv[3] == "-r":
                ...
                if int(ports[0]) < 1:
                    print("[x] starting port range cannnot be under 0")
                    exit()
                elif int(ports[1]) > 65535:
                    print("[x] last port range cannnot be over 65535")
                    exit()
                else:
                    port = int(ports[0])
                    scale = int(ports[1])
...
try:
    waiting_time = ceil((timeout_tcp * 1.5) * (scale - port))
    ...
    portCount = 0
    while port <= scale:
        response = sr1(IP(dst=target_ip) / TCP(dport=port, flags="S"), verbose=False,
                       timeout=timeout_tcp)  # you can augment the timeout if u have a bad connection
        portCount += 1
        if response:
            try:
                if response[TCP].flags == 18:
                    print("[*] Open port: " + str(port)+" ("+getProtocol(port)+")")
                    openPorts += 1
            except IndexError:
                print("[*] Closed/Filtred port: "+ str(port) +" ("+getProtocol(port)+")")
                UnknownPorts += 1
        port += 1
    print("[+] TCP Scan End " + str(portCount) + " port(s) scanned, " + str(openPorts) + " port(s) open, " + str(
        UnknownPorts) + " port(s) filtred/closed")
